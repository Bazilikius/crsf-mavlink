...
class ConfiguratorApp:
    ...
    def create_widgets(self):
        ...
        self.notebook.add(tab_vrx, text="VRX Controller Setup")
    ...
    def toggle_connection(self):
        if self.btn_connect['text'] == "Connect":
            ...
            if self.conn.connect(port):
                ...
                # Request config reading immediately
    ...
    def setup_switcher_tab(self, parent):
        ...
        self.mode_var = tk.IntVar(value=3) # Default simultaneous
    ...
    def setup_vrx_tab(self, parent):
        ...
        lbl_vrx_head.grid(row=0, column=0, columnspan=2, sticky="w", padx=20, pady=15)

        ttk.Label(parent, text="Remote Control Switch Channel:").grid(row=1, column=0, sticky="e", padx=10, pady=10)
        self.vrx_rc_chan_var = tk.IntVar(value=8)
        self.ent_rc_chan = ttk.Entry(parent, textvariable=self.vrx_rc_chan_var, width=10)
        self.ent_rc_chan.grid(row=1, column=1, sticky="w", padx=10, pady=10)

        ttk.Label(parent, text="VRX Selectable Band:").grid(row=2, column=0, sticky="e", padx=10, pady=10)
        ...
        self.band_combo.grid(row=2, column=1, sticky="w", padx=10, pady=10)

        ttk.Label(parent, text="VRX Selectable Channel:").grid(row=3, column=0, sticky="e", padx=10, pady=10)
        ...
        self.chan_combo.grid(row=3, column=1, sticky="w", padx=10, pady=10)

        ttk.Label(parent, text="Direct Frequency (MHz):").grid(row=4, column=0, sticky="e", padx=10, pady=10)
        self.ent_freq = ttk.Entry(parent, width=10)
        self.ent_freq.grid(row=4, column=1, sticky="w", padx=10, pady=10)
        self.ent_freq.insert(0, "5740")

        self.btn_update_vrx = ttk.Button(parent, text="Apply VRX Settings", command=self.on_apply_vrx)
        self.btn_update_vrx.grid(row=5, column=0, columnspan=2, sticky="ew", padx=30, pady=20)
    ...
    def on_config_received(self, config):
        self.log("Received configuration update from board!")
