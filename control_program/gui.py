...
class ConfiguratorApp:
    ...
    def toggle_connection(self):
        if self.btn_connect['text'] == "Connect":
            ...
            try:
                self.conn.udp_port_sec = int(self.mav_remote_port_var.get())
                self.conn.udp_tx_port_sec = int(self.mav_14556_port_var.get())
            except Exception:
                pass
    ...
    def setup_switcher_tab(self, parent):
        ...
        ttk.Label(mav_frame, text="Configure and manually run MAVP2P router process:", font=('Segoe UI', 9, 'bold'), foreground='#1A365D').grid(row=0, column=0, columnspan=2, sticky="w", padx=15, pady=3)

        ttk.Label(mav_frame, text="UDP Forward 1 (19415):").grid(row=1, column=0, sticky="e", padx=15, pady=3)
        ttk.Entry(mav_frame, textvariable=self.mav_remote_port_var, width=15).grid(row=1, column=1, sticky="w", padx=15, pady=3)

        ttk.Label(mav_frame, text="UDP Forward 2 (14556):").grid(row=2, column=0, sticky="e", padx=15, pady=3)
        ttk.Entry(mav_frame, textvariable=self.mav_14556_port_var, width=15).grid(row=2, column=1, sticky="w", padx=15, pady=3)

        ttk.Label(mav_frame, text="MAVP2P Binary Path:").grid(row=3, column=0, sticky="e", padx=15, pady=3)
        ttk.Entry(mav_frame, textvariable=self.mav_bin_path_var, width=25).grid(row=3, column=1, sticky="w", padx=15, pady=3)

        ttk.Button(mav_frame, text="Save MAVP2P Settings", command=self.save_mavp2p_config).grid(row=4, column=0, columnspan=2, pady=10)

        self.btn_start_mavp2p = ttk.Button(mav_frame, text="Start MAVP2P", command=self.start_mavp2p_process)
        self.btn_start_mavp2p.grid(row=5, column=0, padx=10, pady=5)

        self.btn_stop_mavp2p = ttk.Button(mav_frame, text="Stop MAVP2P", command=self.stop_mavp2p_process)
        self.btn_stop_mavp2p.grid(row=5, column=1, padx=10, pady=5)
    ...
    def load_mavp2p_config(self):
        ...
        self.mav_remote_port_var = tk.StringVar(value="19415")
        ...
        if os.path.exists("mavp2p_config.json"):
            try:
                with open("mavp2p_config.json", "r") as f:
                    ...
                    self.mav_remote_port_var.set(str(cfg.get("remote_port", "19415")))
    ...
    def save_mavp2p_config(self):
        ...
        cfg = {
            ...
            "remote_port": self.mav_remote_port_var.get(),
    ...
    def start_mavp2p_process(self):
        ...
        bin_path = self.mav_bin_path_var.get()
        port_14556 = self.mav_14556_port_var.get()
        port_19415 = self.mav_remote_port_var.get()
        port = self.port_var.get()
        if not port:
            port = "COM101"

        # Matches user's exact bat script connecting directly to the serial port
        cmd = [
            bin_path,
            f"udpc:127.0.0.1:{port_19415}",
            f"udpc:127.0.0.1:{port_14556}",
            f"serial:{port}:400000"
        ]
